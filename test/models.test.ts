...
describe('agent-worker models', () => {
	test('CF_MODELS contains expected models', () => {
		...
		expect(CF_MODELS['@cf/meta/llama-3.1-8b-instruct'].provider).toBe('workers-ai');
	});
	...
	test('ALL_MODELS merges both', () => {
		expect(Object.keys(ALL_MODELS).length).toBeGreaterThan(Object.keys(CF_MODELS).length);
	});
